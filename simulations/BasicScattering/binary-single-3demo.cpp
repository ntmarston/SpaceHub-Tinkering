...
int main(int argc, char** argv) {
    ...
    std::fstream ptc_res_file("simulations/BasicScattering/results/2+1_ptcres-3demo.txt", std::ios::out);
    std::fstream ordered_inputs_file("simulations/BasicScattering/results/ordered_inputs-3demo.txt", std::ios::out);
    ...
    size_t n = 20; //sims per parameter will be n^3
    ...
    for(size_t i=0; i < n; ++i)
    {
        ...
        for(size_t j=0; j < 3; ++j){
            ...
            auto azi_j = 0 + j*((2 * consts::pi - 0) / (3-1)); //Saving iterations since 2d sim shows symmetry
    ...
    //g++ -std=c++17 -O3 -pthread simulations/binary-single-3demo.cpp -o simulations/binary-single-3demo
