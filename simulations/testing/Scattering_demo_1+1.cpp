...
int main(int argc, char** argv) {
    ...
    print(ptc_res_file, "time,id,mass,px,py,pz,vx,vy,vz", '\n');
    size_t n = 50;
    for(size_t i=0; i < n; ++i)
    {
        ...
        std::cout << n << ' ' << i << ' ' << (b_max - b_min) << ' ' << b_i << '\n';
        auto orb = orbit::Hyperbolic(p1.mass, p2.mass, v_inf, b_i, 0.0, 0.0, 0.0, r_start, orbit::Hyper::in);
        ...
        args.add_stop_point_operation([&ptc_res_file, &orb, &i](auto& ptc, auto h) {
            // print the end state of the system into file
            // time,pxyz,vxyz for each particle in the system
            ptc_res_file << ptc << '\n';
            //print(ptc_res_file, n, ',', ptc, '\n');
        });

        args.add_stop_point_operation([&orb_res_file, &orb, &i, &b_i](auto& ptc, auto h) {
            // print the end state of the system into file
