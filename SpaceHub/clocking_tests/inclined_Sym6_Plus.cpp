...
using Solver = methods::Sym6_Plus<f, particles::SizeParticles>;
/*----------------------------------------------------------------------------------------------------------------*/
...
int main(int argc, char** argv) {
    ...
    DiskModel::init_from_file("../src/interaction/disk_tab/disk_default_pagn.csv"); //has the discontinuity issue

    // Configure force toggles
    DiskModel::enable_dynamical_friction = true;
    DiskModel::enable_aerodynamic_drag = true;
    ...
    output_filename << "Sym6_Plus" << "-clocktest-" << inclination_deg << ".dat";
    log_filename << "Sym6_Plus" << "-clocktest-" << inclination_deg << ".log";
    ...
    // TimeSlice(op, start, end, N): fires N times at start, start+dt, ..., start+(N-1)*dt
    // dt = (stop_time - 0.1*stop_time) / 9 = 0.1*stop_time
...
// Commands to compile and run (from clocking_tests/):
// g++ -std=c++17 -O3 -pthread inclined_Sym6_Plus.cpp -o ct_Sym6_Plus
// ./ct_Sym6_Plus [inclination_in_degrees]
// Example: ./ct_Sym6_Plus 5.0
