...
int main(int argc, char** argv) {
    ...
    Scalar m1 = 1e8_Ms;  // particle 0 is the central mass
    Scalar m2 = 10_Ms;
    Scalar sma = 5000_AU;  // In self-regulated zone (Q=1 boundary at ~3800 AU)
    auto ecc = 0.5;
    auto inclination = 180_deg;  // Retrograde orbit
    auto longitude_of_ascending_node = 0_deg;
    auto argument_of_periapsis = 0_deg;
    auto true_anomaly = 0_deg;

    Particle p1{m1, 1e-10_AU};
    Particle p2{m2, 2e-7_AU};  // Schwarzschild radius for 10 Msun BH
    ...
    auto stop_time = 1e6_year;
    args.add_stop_condition(stop_time);

    args.add_operation(TimeSlice(DefaultWriter("SpaceHub/test/nick_test/disk_model/AnalyticalTests/DiskModel-SelfReg.dat"), 0_year, stop_time, 500));
...
// g++ -std=c++17 -O3 -pthread test/nick_test/KeplerSimple-DiskModelSelfReg.cpp -o test/nick_test/KeplerSimple-DiskModelSelfReg
// test/nick_test/KeplerSimple-DiskModelSelfReg
