// CN06 eccentricity damping smoketest — low-eccentricity, coplanar orbit at 100 Rg
// Uses DiskMigration with LoweccDamping_CN06 only
// Expected: eccentricity damps from 0.1 over ~37k year timescale
...
int main() {
    ...
    auto orb = orbit::Elliptic(p1.mass, p2.mass, sma, 0.1, 0_deg, 0_deg, 0_deg, 0_deg);
    ...
    args.add_operation(TimeSlice(DefaultWriter("out/cn06.dat"), 0_year, stop_time, 1000));

    tools::Timer timer;
    timer.start();
    solver.run(args);

    print(std::cout, "CN06 test complete in ", timer.get_time(), "s\n");
    return 0;
}

// g++ -std=c++17 -O3 -pthread CN06Test.cpp -o CN06Test
