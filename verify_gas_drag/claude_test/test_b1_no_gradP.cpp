// Phase B1: DiskModel with grad_P zeroed (disables sub-Keplerian correction)
// NO header modifications needed — just post-process disk_table in memory.
...
int main(int argc, char** argv) {
    ...
    // Zero out grad_P to disable sub-Keplerian correction
    for (auto& row : DiskModel::disk_table) {
        row.grad_P = 0.0;
    }
    print(std::cout, "B1: grad_P zeroed for all rows\n");
    ...
    args.add_operation(TimeSlice(DefaultWriter("output/b1_no_gradP.dat"), 0_year, stop_time, 500));

    print(std::cout, "B1: Starting (DiskModel, grad_P=0)\n");
    tools::Timer timer;
    timer.start();
    solver.run(args);
    print(std::cout << "B1: Complete in " << timer.get_time() << "s\n");

    return 0;
}
// g++ -std=c++17 -O3 -pthread test_b1_no_gradP.cpp -o test_b1
// timeout 120 ./test_b1
