// Phase B5: DiskModel with plain Catmull-Rom (no Fritsch-Carlson) for grad_P only
// Tests whether FC non-smoothness on grad_P is the root cause vs. the value of grad_P

#define TEST_GRADP_NO_FC
...
int main(int argc, char** argv) {
    ...
    args.add_operation(TimeSlice(DefaultWriter("output/b5_gradP_no_fc.dat"), 0_year, stop_time, 500));

    print(std::cout, "B5: Starting (plain Catmull-Rom for grad_P, no FC clamping)\n");
    tools::Timer timer;
    timer.start();
    solver.run(args);
    print(std::cout << "B5: Complete in " << timer.get_time() << "s\n");

    return 0;
}
// g++ -std=c++17 -O3 -pthread test_b5_gradP_no_fc.cpp -o test_b5
// timeout 120 ./test_b5
