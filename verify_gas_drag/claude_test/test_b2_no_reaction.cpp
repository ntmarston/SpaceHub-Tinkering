// Phase B2: DiskModel WITHOUT Newton's 3rd law reaction on central mass
// Tests whether acceleration[0] += ... causes the hang

#define TEST_NO_REACTION
...
int main(int argc, char** argv) {
    ...
    args.add_operation(TimeSlice(DefaultWriter("output/b2_no_reaction.dat"), 0_year, stop_time, 500));

    print(std::cout, "B2: Starting (no Newton's 3rd law reaction)\n");
    tools::Timer timer;
    timer.start();
    solver.run(args);
    print(std::cout << "B2: Complete in " << timer.get_time() << "s\n");

    return 0;
}
// g++ -std=c++17 -O3 -pthread test_b2_no_reaction.cpp -o test_b2
// timeout 120 ./test_b2
